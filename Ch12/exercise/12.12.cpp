...
int main()
{
    ...
    // (b) Error. Cannot implicitly convert a ordinary pointer to a smart
    // pointer.
    // process(new int());

    // (c) Error. Cannot implicitly convert a ordinary pointer to a smart
    // pointer.
    // process(p);
    ...
    // function is executed, destory `ptr` and decrease the reference count to
    // 0, thus freed the memory pointed by `ptr`. However, the original ordianry
